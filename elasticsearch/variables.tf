...
locals {
    ...
    pod_annotations = merge({
        ...
        "elasticsearch.instances": "[{\"url\": \"http://%%host%%:9200\", \"cluster_stats\": \"true\", \"index_stats\": \"true\", \"pending_task_stats\": \"true\"}]",
        "elasticsearch.tags": " {\"es_cluster_name\": \"${var.cluster_name}\"} ",
    }, var.pod_annotations)
}
